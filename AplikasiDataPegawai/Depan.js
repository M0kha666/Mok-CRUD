...
class Depan extends Component {
    ...
    constructor(props) {
        ...
        this.url = "http://192.168.1.8/api/api.php";
    }
    componentDidMount(){
        this.ambilListData()
    }
    ...
    klikSimpan(){
        ...
        ''){
            ...
            fetch(urlAksi,{
                ...
                body:"nama="+this.state.ama+"&alamat="+this.state.alamat
            })
            .then((response)=>response.json())
            .then((json)=>{
                this.setState({nama:''});
        ...
        <View style={style.viewData}>
            {
                this.state.listData.map((val,index)=>(
                    ...
                    {index}>
                        ...
                        >{val.nama}</Text>
                        <Text style={style.textListEdit} onPress={()=>this.klikEdit(val.id)}
                        >Edit</Text>
                        <Text style={style.textListDelete} onPress={()=>this.klikDelete(val.id)}
                        >Delete</Text>
                    </View>
                ))
            }
